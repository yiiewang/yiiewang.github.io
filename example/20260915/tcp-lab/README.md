...
# 1) 回声服务器：三种模式任选
go run ./20260915/tcp-lab/echo-server -mode=bufio -addr=:20080

# 2) 代理：把 :8080 转发到 :20080（先起 echo-server 或任意 TCP 服务）
go run ./20260915/tcp-lab/proxy -listen=:8080 -target=127.0.0.1:20080

# 3) 扫描器：扫本机 1-1024
go run ./20260915/tcp-lab/scanner -host=127.0.0.1 -start=1 -end=1024 -workers=256
```
