...
!!! tip "完整代码"
    ...
    本节实现的可运行版本（含基础使用演示、恰好一次投递校验，以及与 channel 的对比压测）在仓库 [example/2026/09/07-1/per-slot](https://github.com/yiiewang/yiiewang.github.io/tree/master/example/2026/09/07-1/per-slot)：
...
- ⚠️ **自旋 vs 休眠**：核多于线程时自旋是优势（不让核心），核超订阅时是灾难（烧 CPU 换空转）。`Gosched()` 是两者的妥协——但这句注释引出的追问（阻塞还是自旋、Gosched 与 park/唤醒的分工、1P 下的实测翻车）见[这篇实测](./09-1.md)
