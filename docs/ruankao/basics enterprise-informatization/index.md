...
!!! tip "记忆技巧"
    六阶段可记为：**"初传控集数成"**（初始→传播→控制→集成→数据管理→成熟）
...
!!! tip "记忆技巧"
    四库 = **知识库 + 数据库 + 模型库 + 方法库**，加上 **人机对话接口** 共五个组成部分。
...
!!! warning "高频考点"
    ...
    **通讯服务 → 信息传递与转化服务 → 应用连接服务 → 流程控制服务**
    ...
    这四层是按照 **"从底层基础到上层业务"** 的架构思路划分的，每一层依赖下一层，层层递进：

    | 层次 | 解决的问题 | 类比 |
    |:---:|:---|:---|
    | **通讯服务**（最底层） | 系统之间能不能"说话"（网络连通、消息传输） | 铺电话线 |
    | **信息传递与转化服务** | 说的"语言"能不能互相理解（数据格式转换、协议适配） | 翻译官 |
    | **应用连接服务** | 不同应用之间能不能互相"调用"（接口适配、服务封装） | 插座适配器 |
    | **流程控制服务**（最高层） | 多个系统如何协同完成一个完整业务流程（工作流编排） | 指挥调度中心 |

    > 企业里有很多异构系统（ERP、CRM、OA……），要让它们集成协作，必须：先保证 **物理上能通信**，再保证 **数据能互相读懂**，再保证 **应用能互相调用**，最后才能 **协调完成完整业务**。缺少任何一层，集成都无法完整实现。
...
| 知识点 | 核心内容 |
|:---:|:---|
| 信息系统发展 | 诺兰 6 阶段：初始→传播→控制→集成→数据管理→成熟 |
| DSS 四库一接口 | 知识库 + 数据库 + 模型库 + 方法库 + 人机对话接口 |
| ERP 三大流 | 物流、资金流、信息流 |
| 专家系统核心 | 推理机 + 知识库 |
| EAI 四层次 | 通讯→信息传递与转化→应用连接→流程控制 |
| 电子商务实体 | 客户、商户、银行、认证中心 |
